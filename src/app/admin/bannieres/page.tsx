import Link from 'next/link';
import { CalendarRange, Copy, Eye, EyeOff, Image as ImageIcon, Plus, Search, Trash2 } from 'lucide-react';
import type { Prisma } from '@prisma/client';
import { BannerForm } from '@/components/admin/MarketingForms';
import { Card, EmptyState } from '@/components/ui/primitives';
import { Disclosure } from '@/components/ui/Disclosure';
import { formatDateShort, formatMoney } from '@/lib/format';
import { prisma } from '@/lib/prisma';
import { contient } from '@/lib/db-search';
import { requirePermission } from '@/lib/auth';
import {
  BANNER_PLACEMENT_LABELS,
  BANNER_PLACEMENT_VALUES,
  POST_STATUS,
  POST_STATUS_LABELS,
  POST_STATUS_VALUES,
  type BannerPlacement,
  type PostStatus,
} from '@/lib/constants';
import {
  changerStatutBannerAction,
  deleteBannerAction,
  dupliquerBannerAction,
  retirerImageAction,
} from '@/server/actions/marketing';
import { etatDiffusion } from '@/server/marketing';
import { optionsMarketing } from '@/server/marketing-options';

export const metadata = { title: 'Bannières' };
export const dynamic = 'force-dynamic';

const TONS: Record<string, string> = {
  ouvert: 'border-emerald-400/30 bg-emerald-400/10 text-emerald-200',
  tension: 'border-amber-400/30 bg-amber-400/10 text-amber-200',
  ferme: 'border-red-400/30 bg-red-400/10 text-red-200',
  neutre: 'chip',
};

const jour = (d: Date | null) => (d ? d.toISOString().slice(0, 10) : '');

export default async function AdminBannieresPage({
  searchParams,
}: {
  searchParams: { q?: string; statut?: string; emplacement?: string };
}) {
  await requirePermission('marketing.manage');

  const q = (searchParams.q ?? '').trim();
  const statut = searchParams.statut ?? '';
  const emplacement = searchParams.emplacement ?? '';

  const where: Prisma.BannerWhereInput = {
    AND: [
      q ? { OR: [{ title: contient(q) }, { subtitle: contient(q) }] } : {},
      statut ? { status: statut } : {},
      emplacement ? { placement: emplacement } : {},
    ],
  };

  const [banners, options, groupes] = await Promise.all([
    prisma.banner.findMany({
      where,
      orderBy: [{ placement: 'asc' }, { sortOrder: 'asc' }, { createdAt: 'desc' }],
      take: 200,
      include: {
        course: { select: { name: true } },
        session: { select: { title: true, course: { select: { name: true } } } },
      },
    }),
    optionsMarketing(),
    prisma.banner.groupBy({ by: ['status'], _count: { _all: true } }),
  ]);

  const compte = (s: string) => groupes.find((g) => g.status === s)?._count._all ?? 0;
  const totalToutes = groupes.reduce((n, g) => n + g._count._all, 0);
  const maintenant = new Date();

  const lien = (modif: Record<string, string>) => {
    const p = new URLSearchParams();
    for (const [k, v] of Object.entries({ q, statut, emplacement, ...modif })) if (v) p.set(k, v);
    return p.toString() ? `/admin/bannieres?${p}` : '/admin/bannieres';
  };

  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="label-eyebrow mb-1">Marketing</p>
          <h1 className="section-title">Bannières</h1>
          <p className="mt-2 max-w-2xl text-sm text-cream-muted">
            Une bannière occupe un emplacement précis du site. Celle de l’en-tête d’accueil passe avant toute
            publication : c’est elle que la cliente voit en premier.
          </p>
        </div>
        <form method="get" className="relative">
          {statut ? <input type="hidden" name="statut" value={statut} /> : null}
          {emplacement ? <input type="hidden" name="emplacement" value={emplacement} /> : null}
          <Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-cream-dim" />
          <input name="q" defaultValue={q} placeholder="Titre de la bannière…" className="w-64 pl-9" />
        </form>
      </div>

      <Disclosure
        label={
          <>
            <Plus size={15} /> Nouvelle bannière
          </>
        }
      >
        <BannerForm formations={options.formations} sessions={options.sessions} />
      </Disclosure>

      <div className="space-y-2">
        <div className="flex flex-wrap gap-2">
          <Link href={lien({ statut: '' })} className={statut === '' ? 'btn-gold px-4 py-2 text-xs' : 'chip'}>
            Toutes ({totalToutes})
          </Link>
          {POST_STATUS_VALUES.map((s) => (
            <Link key={s} href={lien({ statut: s })} className={statut === s ? 'btn-gold px-4 py-2 text-xs' : 'chip'}>
              {POST_STATUS_LABELS[s]} ({compte(s)})
            </Link>
          ))}
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href={lien({ emplacement: '' })}
            className={emplacement === '' ? 'btn-gold px-4 py-2 text-xs' : 'chip'}
          >
            Tous les emplacements
          </Link>
          {BANNER_PLACEMENT_VALUES.map((p) => (
            <Link
              key={p}
              href={lien({ emplacement: p })}
              className={emplacement === p ? 'btn-gold px-4 py-2 text-xs' : 'chip'}
            >
              {BANNER_PLACEMENT_LABELS[p]}
            </Link>
          ))}
        </div>
      </div>

      {banners.length === 0 ? (
        <EmptyState
          title="Aucune bannière"
          description="Créez une bannière pour mettre une session en avant à un endroit précis du site."
        />
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {banners.map((b) => {
            const etat = etatDiffusion({ status: b.status, debut: b.startsAt, fin: b.endsAt }, maintenant);
            const rattachement = b.session ? `${b.session.course.name} — ${b.session.title}` : (b.course?.name ?? null);
            return (
              <Card key={b.id}>
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate font-display text-lg text-cream">{b.title}</p>
                    {b.subtitle ? <p className="text-sm text-cream-muted">{b.subtitle}</p> : null}
                  </div>
                  <span className={`chip shrink-0 ${TONS[etat.ton] ?? ''}`}>{etat.libelle}</span>
                </div>

                <div className="mb-3 flex flex-wrap gap-2 text-xs">
                  <span className="chip text-gold-300">
                    {BANNER_PLACEMENT_LABELS[b.placement as BannerPlacement] ?? b.placement}
                  </span>
                  <span className="chip">{POST_STATUS_LABELS[b.status as PostStatus] ?? b.status}</span>
                  {rattachement ? <span className="chip">{rattachement}</span> : null}
                  {b.price !== null ? <span className="chip text-gold-300">{formatMoney(b.price)}</span> : null}
                  {b.startsAt || b.endsAt ? (
                    <span className="chip">
                      <CalendarRange size={12} />
                      {b.startsAt ? formatDateShort(b.startsAt) : '…'}
                      {' → '}
                      {b.endsAt ? formatDateShort(b.endsAt) : '…'}
                    </span>
                  ) : null}
                  {b.imageUrl ? (
                    <span className="chip">
                      <ImageIcon size={12} /> visuel
                    </span>
                  ) : null}
                </div>

                {b.description ? <p className="mb-3 line-clamp-2 text-sm text-cream-muted">{b.description}</p> : null}

                <div className="mb-2 flex flex-wrap gap-2">
                  {b.status !== POST_STATUS.PUBLIEE ? (
                    <form action={changerStatutBannerAction}>
                      <input type="hidden" name="id" value={b.id} />
                      <input type="hidden" name="status" value={POST_STATUS.PUBLIEE} />
                      <button type="submit" className="btn-outline px-3 py-2 text-xs">
                        <Eye size={13} /> Publier
                      </button>
                    </form>
                  ) : (
                    <form action={changerStatutBannerAction}>
                      <input type="hidden" name="id" value={b.id} />
                      <input type="hidden" name="status" value={POST_STATUS.BROUILLON} />
                      <button type="submit" className="btn-ghost px-3 py-2 text-xs">
                        <EyeOff size={13} /> Dépublier
                      </button>
                    </form>
                  )}
                  {b.status !== POST_STATUS.ARCHIVEE ? (
                    <form action={changerStatutBannerAction}>
                      <input type="hidden" name="id" value={b.id} />
                      <input type="hidden" name="status" value={POST_STATUS.ARCHIVEE} />
                      <button type="submit" className="btn-ghost px-3 py-2 text-xs">
                        Archiver
                      </button>
                    </form>
                  ) : null}
                  <form action={dupliquerBannerAction}>
                    <input type="hidden" name="id" value={b.id} />
                    <button type="submit" className="btn-ghost px-3 py-2 text-xs">
                      <Copy size={13} /> Dupliquer
                    </button>
                  </form>
                  {b.imageUrl ? (
                    <form action={retirerImageAction}>
                      <input type="hidden" name="id" value={b.id} />
                      <input type="hidden" name="type" value="banner" />
                      <button type="submit" className="btn-ghost px-3 py-2 text-xs">
                        Retirer le visuel
                      </button>
                    </form>
                  ) : null}
                </div>

                <div className="space-y-2">
                  <Disclosure variant="row" label="Modifier et prévisualiser">
                    <BannerForm
                      banner={{
                        id: b.id,
                        title: b.title,
                        subtitle: b.subtitle ?? '',
                        description: b.description ?? '',
                        price: b.price === null ? '' : String(b.price),
                        placement: b.placement,
                        courseId: b.courseId ?? '',
                        sessionId: b.sessionId ?? '',
                        ctaLabel: b.ctaLabel ?? '',
                        ctaUrl: b.ctaUrl ?? '',
                        status: b.status,
                        startsAt: jour(b.startsAt),
                        endsAt: jour(b.endsAt),
                        sortOrder: b.sortOrder,
                        imageUrl: b.imageUrl,
                      }}
                      formations={options.formations}
                      sessions={options.sessions}
                    />
                  </Disclosure>
                  <form action={deleteBannerAction}>
                    <input type="hidden" name="id" value={b.id} />
                    <button type="submit" className="btn-danger w-full px-3 py-2 text-xs">
                      <Trash2 size={13} /> Supprimer
                    </button>
                  </form>
                </div>
              </Card>
            );
          })}
        </div>
      )}
    </div>
  );
}
